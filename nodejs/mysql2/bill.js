...
module.exports.insert = function (request, response) {
    ...
    else 
    {
        ...
        let sql = "select p.id,title,p.price,stock,quantity from product p, cart c where billid=0  and usersid=? and c.productid=p.id ";
        let data = [usersid];
        connection.db.query(sql,data,function(error,result){
            if (error) {
                ...
                //check is there any unpaid product in cart (where billid =0)
                if(result.length === 0)
                {
                    ...
                    if (outofstock.length >= 1) {
                        ...
                        connection.db.query(sql, data2, function (error, result2) {
                            if (error) {
                                ...
                                //update cart 
                                orderdetail.map((item) => {
                                    var billid = result2.insertId;
                                    console.log(billid);
                                    sql = `update cart set billid=${billid},price=${item.price} where billid=0 and productid=${item.id} and usersid=${usersid}`;
                                    console.log(sql);
                                    connection.db.query(sql, function (error, result3) {
                                        if (error) {
                                            response.json([{ 'error': 'error occured' }]);
                                            console.log(error);
                                        }
                                ...
                                response.json([{ 'error': 'no' }, { 'success': 'yes' }, { 'message': 'new order created with order id ' + result2.insertId }]);
                            }
                        })
